...
Compare explicit models on a shared agent-authored golden set with `npm run eval:compare -- --models gpt-5.6-sol,gpt-5.6-luna --repeats 2`. This suite uses code checks and saved traces, with no LLM judge. See the [comparison method, measured results and per-turn Codex review](docs/evals/comparison.md).
